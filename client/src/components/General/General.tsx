...
const General = (props: GeneralProps) => {
  ...
  useEffect(() => {
    ...
    if (searchInput.length > 0) {
      const heroResults: HeroType[] | undefined = filtered?.filter(
        (h) => h.name.includes(searchInput) || h.realName.includes(searchInput)
      );
        ...
        <AutoSizer>
          {({ width, height }) => (
            <FixedSizeGrid
              ...
              height={550}
              rowCount={
                Math.round(generalHeroes.length / columnCount) < 1
                  ? 1
                  : Math.round(generalHeroes.length / columnCount)
              }
              rowHeight={S ? 394 : 174}
