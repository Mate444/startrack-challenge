...
const Favorites = (props: FavoriteProps) => {
  ...
  function handleFavoriteHeroes(id: number) {
    const filteredFavoriteHeroes: number[] | undefined = favoriteHeroes.filter(
      (heroId: number) => heroId !== id
    );
    localStorage.setItem(
      "favoritesArray",
      JSON.stringify(filteredFavoriteHeroes)
    );
  ...
  filteredFavorites = useMemo(
    () => sortFavorites(favoriteHeroes, filteredFavorites),
    [favoriteHeroes, filteredFavorites]
  );
    ...
    <div className="favorites-container container">
      ...
      {toggle === "open" && (
        <div key={toggle} className="favorites-wrapper">
          <button className="collapse-btn" onClick={() => setToggle("closed")}>
            ▲
          </button>
          <div className="favorite-heroes">
            {filteredFavorites && filteredFavorites.length > 0 ? (
              filteredFavorites.map((h: HeroType, i: number) => (
                <FavoriteHero
                  key={i}
                  index={i}
                  hero={h}
                  handleFavoriteHeroes={handleFavoriteHeroes}
      ...
      {toggle === "closed" && (
        <div className="favorites-wrapper">
          <button className="collapse-btn" onClick={() => setToggle("open")}>
            ▼
          </button>
        </div>
      )}
